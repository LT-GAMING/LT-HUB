...
local rayfield = library:CreateLib({
    ...
    LoadingSubtitle = "sub to Lt Gaming",
    ConfigurationSaving = {
        Enabled = true,
        FolderName = "Lt Gaming",
        FileName = "settings"
    },
Discord = {
    Enabled = false,
...
KeySettings = {
    Title = "Key System",
    Subtitle = "Key System",
    Note = "Note: This is a key system.",
    SaveKey = true,
    GrabKeyFromSite = false,
    Key = {"key1", "key2"}
}
})
local x = library:CreateTab({
    Name = "Main Tab",
    Icon = "rbxassetid://112045466994309",
    PremiumOnly = false
})
local keySection = x:CreateSection({
    Name = "Key System"
})
...
if not success or not guiParent then
    warn("Không thể tạo GUI - lỗi CoreGui")
    return
end
local SelectMonster = nil
local Sea1, Sea2, Sea3 = false, false, false

-- Thiết lập màu và text loader
...
function TweenObject(object, time, props)
game:GetService("TweenService"):Create(object, TweenInfo.new(time, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut), props):Play()
end
...
function RoundCorner(radius, parent)
...
if not success then return warn("Không thể ghi GUI vào CoreGui!") end
    
    local gui = CreateObject("ScreenGui", {Name = "LTGMLoader", Parent = CoreGui})
    local main = CreateObject("Frame", {
        Name = "Main",
        Parent = gui,
        BackgroundColor3 = shared.LoaderColors.Main,
        BorderSizePixel = 0,
...
--// [4] Giao diện phụ
local userImg = CreateObject("ImageLabel", {
    Name = "UserImage",
    Parent = main,
...
local userName = CreateObject("TextLabel", {
    ...
    TextColor3 = shared.LoaderColors.Title,
    TextSize = 14,
    TextXAlignment = Enum.TextXAlignment.Left
})

--// [5] Thanh loader và animation
...
local bg = CreateObject("Frame", {
    ...
    BackgroundTransparency = 1,
    BackgroundColor3 = shared.LoaderColors.LoaderBackground,
    BorderSizePixel = 0,
...
local progress = CreateObject("Frame", {
    Name = "Progress",
    Parent = bg,
    BackgroundColor3 = shared.LoaderColors.LoaderSplash,
    BackgroundTransparency = 1,
    BorderSizePixel = 0,
    Size = UDim2.new(0, 0, 0, 24)
})
RoundCorner(8, progress)

local stepText = CreateObject("TextLabel", {
    Name = "StepLabel",
    Parent = main,
    BackgroundTransparency = 1,
    ...
    Text = "",
    TextColor3 = shared.LoaderColors.Topic,
    TextSize = 14,
    TextXAlignment = Enum.TextXAlignment.Center,
    AnchorPoint = Vector2.new(0.5, 0.5)
})

TweenObject(main, 0.25, {Size = UDim2.new(0, 346, 0, 121)})
wait()
TweenObject(bg, 0.5, {BackgroundTransparency = 0})
TweenObject(progress, 0.5, {BackgroundTransparency = 0})

local steps = shared.LoaderKeyFrames
for step, data in pairs(steps) do
    wait(data[1])
    TweenObject(progress, 0.5, {Size = UDim2.new(data[2] / 100, 0, 0, 24)})
    stepText.Text = "Bước " .. tostring(step)
end
wait(0.5)
TweenObject(main, 0.25, {Size = UDim2.new(0, 0, 0, 0)})
wait(0.25)
gui:Destroy()
end

--// [6] Auto AFK chống disconnect
function AntiAFK()
...
--// [7] Detect Sea từ PlaceId
local pid = game.PlaceId
if pid == 2753915549 then Sea1 = true
elseif pid == 4442272183 then Sea2 = true
elseif pid == 7449423635 then Sea3 = true
...
if Sea1 then
    if ((v197 == 1) or (v197 <= 9) or ("local SelectMonster = nil")) then
        Ms = "Bandit";
...
warn("Không thể truy cập CoreGui")
end;
...
for prop, value in pairs(props) do
    ...
    v14:Notify({
        Title = "LT GAMING",
        Content = "TẢI XONG SCRIPT ĐANG TRONG QUÁ TRÌNH THỬ NGHIỆM VÀ UPDATE"
        Duration = 10 
    });
end
